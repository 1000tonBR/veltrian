...
function renderQuoteOptions(values = []) {
  quoteOptions.innerHTML = [0, 1, 2].map((index) => {
    const value = values[index] || {}; const requiredLabel = index === 0 ? 'obrigatório' : 'opcional';
...
quoteOptions.addEventListener('input', updateNetValues); requestSelect.addEventListener('change', updateRequestSummary); quoteCancel.addEventListener('click', resetQuoteForm);
...
quoteForm.addEventListener('submit', async (event) => {
  ...
  const rawOptions = [0, 1, 2].map((index) => ({ supplier_id: form.get(`supplier_${index}`), quoted: form.get(`quoted_${index}`), discount: form.get(`discount_${index}`), delivery_date: form.get(`delivery_${index}`), freight_type: form.get(`freight_${index}`), payment_terms: form.get(`payment_${index}`) }));
