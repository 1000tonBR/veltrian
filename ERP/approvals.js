...
const approvalNumber = (value) => Number(value || 0).toLocaleString('pt-BR', { minimumFractionDigits: 0, maximumFractionDigits: 3 });
...
const approvalQuantity = (request) => request?.lines?.map((line) => approvalNumber(line.quantity)).filter(Boolean).join(', ') || '—';
const approvalUnit = (request) => request?.lines?.map((line) => line.item?.unit_of_measure || '—').join(', ') || '—';
const canApprove = () => ['administrador', 'aprovador'].includes(approvalProfile?.role);
const approvalOnlineQuoteLabels = { aguardando_envio: 'Aguardando envio', enviada: 'Enviada', entregue: 'Entregue', acessada: 'Acessada', respondida: 'Respondida', expirada: 'Expirada', erro_envio: 'Erro no envio', cancelada: 'Cancelada', resposta_tardia: 'Resposta após encerramento' };
const approvalQuoteOrigin = (quote) => quote?.origin === 'online' || quote?.online_invitation_id ? 'Online' : 'Manual';
const approvalQuoteProgress = (quote) => approvalQuoteOrigin(quote) === 'Online' ? approvalOnlineQuoteLabels[quote?.invitation?.status] || 'Aguardando envio' : 'Preenchida';
...
function renderApprovalOrders() {
  ...
  body.innerHTML = entries.length ? entries.map((order) => `<tr><td><strong>${approvalOrderCode(order.order_number)}</strong></td><td>${approvalRcCode(order.request?.request_number)}</td><td>${approvalEscape(order.supplier?.legal_name)}</td><td>${approvalEscape(approvalMaterial(order.request))}</td><td><strong>${approvalEscape(approvalQuantity(order.request))}</strong></td><td>${approvalEscape(approvalUnit(order.request))}</td><td><strong>${approvalMoney(order.total_value)}</strong></td><td>${approvalEscape(approvalPerson(order.request?.requester))}</td><td>${approvalEscape(approvalPerson(order.creator))}</td><td>${approvalStatus(order.status)}</td><td>${approvalDate(order.created_at)}</td><td><button type="button" class="row-button" data-review-order="${order.id}">${order.status === 'em_aprovacao' ? 'Analisar' : 'Ver detalhes'}</button></td></tr>`).join('') : '<tr><td colspan="12" class="empty-cell">Nenhum pedido corresponde ao filtro.</td></tr>';
}
