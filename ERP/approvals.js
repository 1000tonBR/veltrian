...
function renderApprovalOrders() {
  ...
  body.innerHTML = entries.length ? entries.map((order) => `<tr><td><strong>${approvalOrderCode(order.order_number)}</strong></td><td>${approvalRcCode(order.request?.request_number)}</td><td>${approvalEscape(order.supplier?.legal_name)}</td><td>${approvalEscape(approvalMaterial(order.request))}</td><td>${approvalEscape(approvalUnit(order.request))}</td><td><strong>${approvalMoney(order.total_value)}</strong></td><td>${approvalEscape(approvalPerson(order.request?.requester))}</td><td>${approvalEscape(approvalPerson(order.creator))}</td><td>${approvalStatus(order.status)}</td><td>${approvalDate(order.created_at)}</td><td><button type="button" class="row-button" data-review-order="${order.id}">${order.status === 'em_aprovacao' ? 'Analisar' : 'Ver detalhes'}</button></td></tr>`).join('') : '<tr><td colspan="11" class="empty-cell">Nenhum pedido corresponde ao filtro.</td></tr>';
}
