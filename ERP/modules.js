...
async function loadItems() {
  const { data, error } = await client.from('items').select('*').order('description');
...
async function loadActivities() {
  const { data, error } = await client.from('activities').select('*').eq('active', true).order('code');
...
async function loadRequests() {
  const { data, error } = await client.from('purchase_requests').select('id, request_number, priority, status, activity:activities(code,description), lines:purchase_request_items(quantity,item:items(description))').order('created_at', { ascending: false });
...
document.querySelector('[data-activity-form]')?.addEventListener('submit', async (event) => {
  event.preventDefault();
  const form = new FormData(event.currentTarget);
  const { error } = await client.from('activities').insert({ code: String(form.get('code')).trim().toUpperCase(), description: form.get('description') });
...
document.querySelector('[data-rc-form]')?.addEventListener('submit', async (event) => {
  ...
  const { data: rc, error } = await client.from('purchase_requests').insert({ title: item?.description || 'Requisição de compra', description: form.get('notes') || null, category: 'Compras', estimated_value: 0, requested_by: user.id, priority: form.get('priority'), activity_id: form.get('activity_id') || null }).select('id').single();
  if (error) return showNotice(`RC não criada: ${error.message}`, 'error');
  const { error: lineError } = await client.from('purchase_request_items').insert({ purchase_request_id: rc.id, item_id: form.get('item_id'), quantity: Number(form.get('quantity')), notes: form.get('notes') || null });
