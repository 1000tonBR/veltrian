...
function renderRequests(entries = requests) {
  setRows('[data-requests-rows]', entries, 'Nenhuma requisição criada até o momento.', (rc) => {
    ...
    return `<tr class="${completed ? 'order-issued-row' : ''}"><td>${rcCode(rc.request_number)}${rc.origin === 'automatic' ? '<small class="procurement-reference">Automática</small>' : ''}</td><td><strong>${orderCode(order?.order_number)}</strong></td><td>${materialCode(line?.item?.material_number)}</td><td>${escapeHtml(material)}</td><td>${escapeHtml(line?.item?.unit_of_measure)}</td><td>${escapeHtml(line?.quantity)}</td><td><span class="priority ${escapeHtml(rc.priority)}">${escapeHtml(rc.priority)}</span></td><td>${escapeHtml(activity)}</td><td>${escapeHtml(requester)}</td><td>${workflowStatus}</td><td>${formatDate(rc.created_at)}</td><td class="table-actions"><button type="button" class="row-button" data-edit-rc="${rc.id}">Editar</button><button type="button" class="row-button danger" data-delete-rc="${rc.id}">Excluir</button></td></tr>`;
  }, 12);
}
...
async function loadRequests() {
  const { data, error } = await moduleClient.from('purchase_requests').select('id, request_number, title, description, priority, status, activity_id, requested_by, origin, created_at, requester:profiles!purchase_requests_requested_by_fkey(full_name,email), activity:activities(code,description), lines:purchase_request_items(id,item_id,quantity,notes,item:items(id,description,material_number,unit_of_measure)), orders:purchase_orders!purchase_orders_purchase_request_id_fkey(order_number,status,created_at)').order('created_at', { ascending: false });
...
function applyRcFilter() {
  ...
  const fieldsFor = (rc) => ({
    ...
    all: [rcCode(rc.request_number), rc.request_number, rc.origin === 'automatic' ? 'automática' : 'manual', ...((rc.orders || []).flatMap((order) => [orderCode(order.order_number), order.order_number, order.status])), materialCode(rc.lines?.[0]?.item?.material_number), rc.lines?.[0]?.item?.material_number, rc.lines?.[0]?.item?.description, rc.activity?.code, rc.activity?.description, rc.requester?.full_name, rc.requester?.email]
  });
