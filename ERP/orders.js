...
async function loadOrders() {
  const { data, error } = await orderClient.from('purchase_orders').select('*,supplier:suppliers(*),quote:quotes(*),request:purchase_requests(id,request_number,description,activity:activities(code,description),lines:purchase_request_items(quantity,notes,item:items(description,manufacturer,serial_number,material_number,unit_of_measure)))').order('created_at', { ascending: false });
